...
void loop() {
  ...
  String command = getValue(computerText, ' ',0);
    ...
    if (command == "right" || command == "رايت" || command == "يمين") {
      base.write(base_angle -= 20);
    }
    if (command == "left" || command == "لفت" || command == "يسار") {
     base.write(base_angle += 20);
    }

    if (command == "top" || command == "توب" || command == "اعلى") {
      shoulder.write(shoulder_angle -= 20);
    }

   if (command == "bottom"|| command == "بوتوم" || command == "اسفل") {
     shoulder.write(shoulder_angle += 20);
    }
